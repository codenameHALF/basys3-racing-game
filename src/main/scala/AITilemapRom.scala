...
// Separate copy of TilemapRom for the AI
class AITilemapRom(BackTileNumber: Int, SpriteNumber: Int, TilemapNumber: Int) extends Module {
  ...
  val tilemapMemories = for (i <- 0 until TilemapNumber) yield {
    Module(new Memory(tileWidth, addrWidth, "memory_init/tilemap_init_" + i.toString + ".mem"))
  }
